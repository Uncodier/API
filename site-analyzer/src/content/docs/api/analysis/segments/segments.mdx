...
> 🚀 **Potencia tu estrategia de segmentación** 
> 
> Esta API utiliza modelos avanzados de IA para identificar segmentos de audiencia altamente rentables, 
> analizar su comportamiento y proponer estrategias de monetización específicas para cada uno.
...
- Captura y análisis de capturas de pantalla del sitio web
...
<UnifiedApiTester 
  ...
  additionalFields={[
    ...
    },
    {
      ...
      type: "select",
      options: [
        { value: "analyze", label: "Análisis (sin crear segmentos)" },
        { value: "create", label: "Creación (crear segmentos nuevos)" },
        { value: "update", label: "Actualización (actualizar segmentos existentes)" }
      ],
    ...
    },
    {
      ...
      label: "Puntuación mínima de confianza",
      type: "number",
      defaultValue: 0.7,
      min: 0,
      max: 1,
    ...
    },
    {
      ...
      type: "multiselect",
      options: [
        { value: "demographic", label: "Demográficos" },
        { value: "behavioral", label: "Comportamiento" },
        { value: "psychographic", label: "Psicográficos" },
        { value: "technographic", label: "Tecnográficos" },
        { value: "firmographic", label: "Firmográficos (B2B)" }
      ],
...
> 
> Esta API utiliza internamente la API de Conversación para analizar el sitio web.
> Esto permite aprovechar todas las capacidades de los modelos de IA multimodales,
> incluyendo el análisis de capturas de pantalla y contenido HTML del sitio.
> Cuando se proporciona una URL, siempre se realiza un análisis real del sitio.

El flujo de procesamiento de la API es el siguiente:

1. **Recepción de parámetros**: La API recibe la URL del sitio y los parámetros de configuración
2. **Captura de datos**: Se obtiene el HTML y opcionalmente una captura de pantalla del sitio
3. **Análisis con IA**: Se utiliza la API de Conversación para analizar los datos con el modelo seleccionado
4. **Procesamiento de resultados**: Se estructuran los segmentos identificados
5. **Operaciones en base de datos**: Según el modo seleccionado, se crean o actualizan segmentos
6. **Respuesta estructurada**: Se devuelve un JSON con los segmentos y metadatos del análisis

## Modos de operación

> 💡 **Selecciona el modo adecuado para tu caso de uso**
> 
> La API ofrece tres modos de operación que determinan cómo se procesan los segmentos identificados.
> Elige el que mejor se adapte a tus necesidades de integración y flujo de trabajo.

| Modo | Parámetro | Descripción | Caso de uso típico |
|------|-----------|-------------|-------------------|
| **Análisis** | `mode: "analyze"` | Analiza el sitio web y devuelve los segmentos más rentables identificados, sin crearlos en la base de datos. | Cuando solo necesitas información sobre segmentos potenciales sin almacenarlos. |
| **Creación** | `mode: "create"` | Analiza el sitio web, identifica los segmentos más rentables y los crea automáticamente en la base de datos del usuario si no existen. | Para iniciar una estrategia de segmentación desde cero. |
| **Actualización** | `mode: "update"` | Analiza el sitio web, identifica los segmentos más rentables y actualiza los existentes en la base de datos con nueva información. | Para mantener tus segmentos actualizados con los últimos datos. |

## Métodos disponibles

### POST /api/site/segments

Analiza un sitio web para identificar los segmentos más rentables y opcionalmente los crea en la base de datos según el modo de operación seleccionado.
...
```json
{
  ...
  "includeScreenshot": true, // Incluir captura de pantalla en el análisis
  ...
  "additionalInstructions": "Enfocarse en segmentos con alto potencial de compra recurrente y analizar patrones de uso en dispositivos móviles. Incluir información detallada sobre perfiles de audiencia para plataformas publicitarias como Google Ads, Facebook Ads, LinkedIn Ads y TikTok Ads.", // Indicaciones adicionales en formato texto
  "aiProvider": "openai", // Proveedor de IA: 'openai', 'anthropic', 'gemini'
  "aiModel": "gpt-4o" // Modelo específico de IA a utilizar
}
```

> 📝 **Nota sobre los IDs de usuario y sitio**
> 
> Los campos `user_id` y `site_id` permiten asociar los segmentos analizados con un usuario y sitio específicos
> en tu sistema. Esto es especialmente útil cuando utilizas los modos `create` o `update` para gestionar
> segmentos en la base de datos. Si no se proporcionan, se utilizará un ID temporal para operaciones sin
> autenticación.
  ...
  "segments": [
    {
      ...
      "summary": "Segmento altamente rentable de creadores digitales con necesidades específicas de herramientas profesionales.",
    ...
    },
    {
      ...
      "summary": "Segmento premium con alto poder adquisitivo que realiza compras desde dispositivos móviles.",
...
> 
> A continuación se muestra el prompt exacto que se envía al modelo de IA para el análisis de segmentos.
> Este prompt está diseñado para obtener una respuesta estructurada en formato JSON con información
> detallada sobre los segmentos más rentables para el sitio web analizado.
...
> 
> El prompt anterior es la versión básica. La API permite personalizar varios aspectos:
> 
> - **Métricas de rentabilidad**: Se pueden especificar métricas específicas para evaluar la rentabilidad
> - **Atributos de segmentos**: Se pueden solicitar atributos específicos para cada segmento
> - **Contexto de industria**: Se puede proporcionar información sobre la industria del sitio
> - **Instrucciones adicionales**: Se pueden añadir instrucciones específicas para el análisis
> - **Modo de operación**: Se pueden incluir instrucciones específicas según el modo (create/update)

## Formato de respuesta esperado

> 🔍 **Estructura de la respuesta**
> 
> La respuesta del modelo de IA debe seguir una estructura JSON específica que incluye información
> detallada sobre cada segmento identificado. A continuación se muestra un ejemplo de la estructura
...
Si se proporciona una URL, genera nuevos segmentos basados en el análisis real de esa URL.
...
| segmentCount | number | Número de segmentos a crear (1-20) | No | 5 |
| userId | string | ID del usuario para asociar los segmentos | No | system_m2m_user |
| debug | boolean | Si es true, incluye información adicional de depuración | No | false |

> 📝 **Nota importante sobre el análisis**
> 
> Cuando se proporciona una URL, la API siempre realiza un análisis real del sitio web.
> El análisis incluye la captura del HTML y una captura de pantalla del sitio, y utiliza
> modelos avanzados de IA para identificar los segmentos más rentables.
> 
...
```json
{
  ...
  "segments": [
    {
      ...
      "name": "Creadores de Contenido Digital",
      "description": "Profesionales y aficionados de 20-40 años dedicados a la creación de contenido digital para redes sociales y plataformas online",
      "summary": "Segmento altamente rentable de creadores digitales con necesidades específicas de herramientas profesionales y disposición a invertir en soluciones que mejoren su flujo de trabajo creativo.",
      "estimatedSize": "15%",
      "estimated_value": "9,000,000",
  ...
  "nextSteps": [
    {
      "action": "Implementar campañas específicas para el segmento de creadores de contenido",
      "priority": "high",
...
> 📊 **Persistencia automática de segmentos**
> 
> La API no solo identifica segmentos rentables, sino que también puede crearlos y actualizarlos 
> automáticamente en tu base de datos. Esto te permite construir un repositorio de segmentos 
...
> 
> Los siguientes ejemplos te muestran cómo integrar la API de Segmentos Rentables en tu aplicación.
> Copia, pega y adapta estos fragmentos de código para comenzar a identificar y monetizar 
> segmentos de audiencia valiosos en cuestión de minutos.
...
const response = await fetch('https://tudominio.com/api/site/segments', {
  ...
  body: JSON.stringify({
    ...
    additionalInstructions: "Enfocarse en segmentos con alto potencial de compra recurrente y analizar patrones de uso en dispositivos móviles.",
    aiProvider: 'openai',
    aiModel: 'gpt-4o',
    timeout: 45000
  })
...
const response = await fetch('https://tudominio.com/api/site/segments', {
  ...
  body: JSON.stringify({
    ...
    mode: 'create',
    includeScreenshot: true, // Incluir captura de pantalla para análisis visual
    profitabilityMetrics: ['conversionRate', 'aov', 'ltv'],
    segmentAttributes: ['demographic', 'behavioral', 'psychographic'],
    industryContext: 'ecommerce',
    additionalInstructions: "Identificar segmentos con alto potencial de monetización a través de suscripciones y analizar patrones de uso en dispositivos móviles. Incluir información detallada sobre perfiles de audiencia para plataformas publicitarias como Google Ads, Facebook Ads, LinkedIn Ads y TikTok Ads.",
...
const response = await fetch('https://tudominio.com/api/site/segments', {
  ...
  body: JSON.stringify({
    ...
    additionalInstructions: "Actualizar información de targeting específica para cada segmento incluyendo audiencias detalladas para Google Ads, Facebook Ads, LinkedIn Ads y TikTok Ads. Enfocarse en los cambios de comportamiento de los segmentos en el último trimestre.",
    aiProvider: 'gemini',
...
console.log(data.segments); // Información actualizada de segmentos
``` 