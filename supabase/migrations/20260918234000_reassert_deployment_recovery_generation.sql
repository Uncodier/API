-- Rollback: reapply the previous definition of
-- public.recover_ready_deployment_infrastructure(uuid, uuid, uuid, text, text, text, text, text, boolean).
CREATE OR REPLACE FUNCTION public.recover_ready_deployment_infrastructure(
  p_requirement_id uuid, p_site_id uuid, p_instance_id uuid, p_branch text,
  p_commit_sha text, p_deployment_id text, p_preview_url text,
  p_recovery_id text, p_allow_legacy boolean DEFAULT false
)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE
  v_requirement_status text; v_requirement_metadata jsonb;
  v_requirement_site_id uuid; v_blocker_provenance text; v_blocker_event_id text;
  v_blocker_plan_id text; v_blocker_step_id text;
  v_blocker_generation integer; v_blocker_step_matched boolean := false;
  ...
  v_plan record; v_entry record; v_steps jsonb; v_step jsonb;
  v_new_step jsonb; v_correlation jsonb; v_structured_match boolean;
  v_legacy_match boolean;
  ...
  v_plan_ids jsonb := '[]'::jsonb; v_step_ids jsonb := '[]'::jsonb;
  v_audit_cycle text;
BEGIN
  IF NULLIF(btrim(p_branch), '') IS NULL
    OR NULLIF(btrim(p_commit_sha), '') IS NULL
    OR NULLIF(btrim(p_recovery_id), '') IS NULL
  THEN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      'state', 'missing', 'matched', false, 'recovered', false,
  ...
  IF v_requirement_status IN ('done', 'cancelled', 'canceled', 'on-review') THEN
    RETURN jsonb_build_object(
      'state', 'guarded', 'matched', false, 'recovered', false,
      'requirement_reopened', false, 'plan_ids', v_plan_ids, 'step_ids', v_step_ids
    );
  END IF;
  ...
  THEN
    RETURN jsonb_build_object(
      'state', 'product_blocked', 'matched', false, 'recovered', false,
      'requirement_reopened', false, 'plan_ids', v_plan_ids, 'step_ids', v_step_ids
    );
  END IF;
  PERFORM 1
  ...
  LOOP
    ...
    LOOP
      ...
      v_legacy_match := COALESCE((
        v_legacy_audited
        AND (v_correlation IS NULL OR v_correlation = 'null'::jsonb)
  ...
  IF v_requirement_status = 'blocked' AND (
    ...
    RETURN jsonb_build_object(
      'state', 'guarded', 'matched', true, 'recovered', false,
      'requirement_reopened', false, 'plan_ids', v_plan_ids, 'step_ids', v_step_ids
    );
  END IF;
  FOR v_plan IN
  ...
  END IF;
  RETURN jsonb_build_object(
    'state', 'applied', 'matched', true, 'recovered', true,
