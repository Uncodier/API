 ...
 *   1. Project id match against `APPS_VERCEL_PROJECT_ID` (for applications) / `VERCEL_PROJECT_ID_AUTOMATION`
 ...
 *
 * NOTE: we read `APPS_VERCEL_PROJECT_ID` (not `VERCEL_PROJECT_ID`) because the
 * latter is a Vercel System Environment Variable that gets auto-injected with
 * the id of the running project (the API project) at runtime, which would
 * always misclassify webhooks coming from the apps project.
 */
export function classifyGitRepoKind(payload: VercelWebhookPayloadBody | undefined): GitRepoKind {
  ...
  const appsProject = process.env.APPS_VERCEL_PROJECT_ID?.trim();
