...
export async function getSegmentById(segmentId: string): Promise<DbSegment | null> {
  try {
    // Importar la función de la capa de base de datos
    const { getSegmentById: getSegmentByIdFromDb } = await import('@/lib/database/segment-db');
    
    // Obtener el segmento de la base de datos
    const dbSegment = await getSegmentByIdFromDb(segmentId);
    
    if (dbSegment) {
      // Adaptar el resultado a nuestra estructura local de DbSegment
      const segment: DbSegment = {
        ...dbSegment,
        created_at: dbSegment.created_at || new Date().toISOString(),
        updated_at: dbSegment.updated_at || new Date().toISOString(),
        // Propiedades adicionales de nuestra interfaz local
        profitability_score: 0.5,
        campaigns: 0,
        is_ai_generated: false
      };
      
      console.log(`[Segment Service] Segmento encontrado: ${segment.name} (ID: ${segment.id})`);
      return segment;
    } else {
      console.log(`[Segment Service] Segmento no encontrado: ${segmentId}`);
      return null;
    }
  } catch (error: any) {
    console.error(`[Segment Service] Error al obtener segmento: ${error.message}`);
