...
export class DataWorkflowService extends BaseWorkflowService {
  ...
  /**
   * Ejecuta el workflow para minería del Perfil de Cliente Ideal (ICP)
   */
  public async idealClientProfileMining(args: { site_id: string }, options?: WorkflowExecutionOptions): Promise<WorkflowExecutionResponse> {
    try {
      if (!args.site_id) {
        return {
          ...
          error: {
            ...
            message: 'Se requiere site_id para minería de ICP'
          }
        };
      }
      ...
      const workflowId = options?.workflowId || `ideal-client-profile-mining-${args.site_id}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      const taskQueue = options?.taskQueue || process.env.WORKFLOW_TASK_QUEUE || 'default';

      console.log(`🧭 Iniciando workflow de minería ICP: ${workflowId}`);
      ...
      console.error('❌ Error al ejecutar workflow de minería ICP:', error);
      return {
        success: false,
        error: {
          code: 'WORKFLOW_EXECUTION_ERROR',
          message: error instanceof Error ? error.message : 'Error desconocido al ejecutar workflow de minería ICP'
        }
      };
    }
  }
  ...
  /**
   * Ejecuta el workflow para enriquecimiento de leads
   */
    ...
    try {
      if (!args.site_id) {
        return {
          ...
          error: {
            code: 'INVALID_ARGUMENTS',
            message: 'Se requiere site_id para el enriquecimiento de lead'
          }
        };
      }

      if (!args.linkedin_profile && !args.person_id) {
        return {
          success: false,
          error: {
            code: 'INVALID_ARGUMENTS',
            message: 'Se requiere al menos linkedin_profile o person_id para el enriquecimiento de lead'
          }
        };
      }
      ...
      const workflowId = options?.workflowId || `enrich-lead-${args.site_id}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      const taskQueue = options?.taskQueue || process.env.WORKFLOW_TASK_QUEUE || 'default';

      console.log(`🔍 Iniciando workflow de enriquecimiento de lead: ${workflowId}`);

      if (options?.async !== false) {
        const handle = await client.workflow.start('enrichLeadWorkflow', {
          args: [args],
          taskQueue,
          workflowId,
        });
        ...
        const result = await client.workflow.execute('enrichLeadWorkflow', {
          args: [args],
          taskQueue,
          workflowId,
        });
      ...
      console.error('❌ Error al ejecutar workflow de enriquecimiento de lead:', error);
      return {
        success: false,
        error: {
          code: 'WORKFLOW_EXECUTION_ERROR',
          message: error instanceof Error ? error.message : 'Error desconocido al ejecutar workflow de enriquecimiento de lead'
        }
      };
    }
  }
    ...
    try {
      if (!args.site_id) {
        return {
          ...
          error: {
            code: 'INVALID_ARGUMENTS',
            message: 'site_id is required for the lead qualification workflow'
          }
        };
      }
      ...
      const workflowId = options?.workflowId || `lead-qualification-${args.site_id}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      const taskQueue = options?.taskQueue || process.env.WORKFLOW_TASK_QUEUE || 'default';

      console.log(`🎯 Iniciando workflow de calificación de leads: ${workflowId}`);
      ...
      if (options?.async !== false) {
        ...
        const result = await client.workflow.execute('leadQualificationWorkflow', {
          args: [args],
          taskQueue,
          workflowId,
        });
      ...
      console.error('❌ Error al ejecutar workflow de calificación de leads:', error);
      return {
        success: false,
        error: {
          code: 'WORKFLOW_EXECUTION_ERROR',
          message: error instanceof Error ? error.message : 'Error desconocido al ejecutar workflow de calificación de leads'
        }
      };
    }
  }
