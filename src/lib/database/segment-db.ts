 ...
 * 
 * @param segmentId ID del segmento a buscar
 * @returns El segmento encontrado o null si no existe
 */
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        // Este error indica que no se encontró el registro
        return null;
      }
      
      console.error('Error al obtener segmento por ID:', error);
      throw new Error(`Error al obtener segmento: ${error.message}`);
    }
    
    return data;
  } catch (error: any) {
    console.error('Error en getSegmentById:', error);
    throw new Error(`Error al obtener segmento por ID: ${error.message}`);
  }
} 