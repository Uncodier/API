...
export class OpenAIAgentExecutor {
  ...
  async act(options: ActOptions): Promise<ActResponse> {
    ...
    // GPT-5.2 (o-series) can handle multiple images, so we keep last N screenshots for context
    ...
    while (iterations < maxIterations) {
      ...
      try {
        ...
        if (message.tool_calls && message.tool_calls.length > 0) {
          ...
          // GPT-5.2 can handle multiple images, so we send the full history (last N screenshots)
