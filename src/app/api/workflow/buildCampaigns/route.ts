...
interface BuildCampaignWorkflowArgs {
  site_id: string;
}
...
/**
 * API endpoint para ejecutar el workflow buildCampaignsWorkflow en Temporal
 * POST /api/workflow/buildCampaign
 */
export async function POST(request: NextRequest) {
  try {
    console.log('🚀 Iniciando ejecución del workflow buildCampaignsWorkflow');
    ...
    const workflowArgs: BuildCampaignWorkflowArgs = {
      site_id
    };
    ...
    const workflowOptions: WorkflowExecutionOptions = {
      ...
      workflowId: `build-campaign-${site_id}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
    };

    console.log(`🔄 Iniciando workflow con ID: ${workflowOptions.workflowId}`);

    // Ejecutar el workflow específico para construir campañas
    const result = await workflowService.buildCampaigns(
      workflowArgs,
      workflowOptions
    );
    ...
    console.error('❌ Error en el endpoint del workflow buildCampaign:', error);
...
export async function GET() {
  return NextResponse.json({
    name: 'buildCampaignsWorkflow API',
    description: 'Ejecuta el workflow buildCampaignsWorkflow en Temporal',
    methods: ['POST'],
    requiredParams: {
      site_id: 'string - ID del sitio para el cual construir campañas'
    },
    example: {
      site_id: 'site_12345'
    }
