...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (site_id) {
      try {
        const requiredCredits = CreditService.PRICING.ENRICHMENT_PHONE;
    ...
    // Build payload (no do_contacts_enrichment field for phone numbers)
    ...
    const url = `https://api-v2.forager.ai/api/${encodeURIComponent(
      foragerAccountId
    )}/datastorage/person_contacts_lookup/phone_numbers/`;

    logInfo('finder.person_contacts_lookup.phone_numbers', 'Upstream request', { url, body: payload });
    ...
    if (!upstreamResponse.ok) {
      ...
      logError('finder.person_contacts_lookup.phone_numbers', 'Upstream error', {
        url,
        params: payload,
        upstream: {
      ...
      return NextResponse.json(
        { error: 'Phone numbers lookup service temporarily unavailable', debug },
        { status: upstreamResponse.status }
      );
    }
    ...
    logError('finder.person_contacts_lookup.phone_numbers', 'Handler exception', error instanceof Error ? { message: error.message, stack: error.stack, params: requestBody } : { error, params: requestBody });
    return NextResponse.json(
