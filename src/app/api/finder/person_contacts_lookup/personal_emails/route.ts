...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Build payload (no do_contacts_enrichment field for personal emails)
    ...
    const url = `https://api-v2.forager.ai/api/${encodeURIComponent(
      foragerAccountId
    )}/datastorage/person_contacts_lookup/personal_emails/`;

    logInfo('finder.person_contacts_lookup.personal_emails', 'Upstream request', { url, body: payload });
    ...
    if (!upstreamResponse.ok) {
      ...
      logError('finder.person_contacts_lookup.personal_emails', 'Upstream error', {
        url,
        params: payload,
        upstream: {
      ...
      return NextResponse.json(
        { error: 'Personal emails lookup service temporarily unavailable', debug },
        { status: upstreamResponse.status }
      );
    }
    ...
    logError('finder.person_contacts_lookup.personal_emails', 'Handler exception', error instanceof Error ? { message: error.message, stack: error.stack, params: requestBody } : { error, params: requestBody });
    return NextResponse.json(
