...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[API:public:videos] Incoming request:', {
      params: {
    ...
    if (siteId) {
      console.log('[API:public:videos] Resolving site context for ID:', { siteId });
      ...
      console.log('[API:public:videos] Supabase search result for siteId:', { sites, siteError });

      if (siteError) {
        console.error('[API:public:videos] Error querying site by ID:', siteError);
        return NextResponse.json({ error: 'Error validating site ID' }, { status: 500 });
      }

      if (!sites || sites.length === 0) {
        console.warn('[API:public:videos] Site not found for the provided site_id:', { siteId });
    ...
    console.log('[API:public:videos] Fetching content with parameters:', {
      siteId,
      limit,
      tag: tagParam,
    ...
    console.error('[API:public:videos] Error:', error);
