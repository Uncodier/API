  ...
  try {
    ...
    if (siteId) {
      ...
      try {
        ...
        console.error('[API:public:landing-pages] Error parsing URL:', e);
      }

      console.log('[API:public:landing-pages] Resolving site context for URL:', { siteUrl, hostname });
      ...
      console.log('[API:public:landing-pages] Supabase search result for site:', { sites, siteError, hostnameSearched: `%${hostname}%` });

      if (siteError) {
        console.error('[API:public:landing-pages] Error querying site by URL:', siteError);
        return NextResponse.json({ error: 'Error validating site URL' }, { status: 500 });
      }

      if (!sites || sites.length === 0) {
        console.warn('[API:public:landing-pages] Site not found for the provided URL:', { siteUrl, hostname });
