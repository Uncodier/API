...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[API:public:ads] Incoming request:', {
      params: {
    ...
    if (siteId) {
      console.log('[API:public:ads] Resolving site context for ID:', { siteId });
      ...
      console.log('[API:public:ads] Supabase search result for siteId:', { sites, siteError });

      if (siteError) {
        console.error('[API:public:ads] Error querying site by ID:', siteError);
        return NextResponse.json({ error: 'Error validating site ID' }, { status: 500 });
      }

      if (!sites || sites.length === 0) {
        console.warn('[API:public:ads] Site not found for the provided site_id:', { siteId });
      ...
      try {
        ...
        console.error('[API:public:ads] Error parsing URL:', e);
      }

      console.log('[API:public:ads] Resolving site context for URL:', { siteUrl, hostname });
      ...
      console.log('[API:public:ads] Supabase search result for site:', { sites, siteError, hostnameSearched: `%${hostname}%` });

      if (siteError) {
        console.error('[API:public:ads] Error querying site by URL:', siteError);
        return NextResponse.json({ error: 'Error validating site URL' }, { status: 500 });
      }

      if (!sites || sites.length === 0) {
        console.warn('[API:public:ads] Site not found for the provided URL:', { siteUrl, hostname });
    ...
    console.log('[API:public:ads] Fetching content with parameters:', {
      siteId,
      limit,
      tag: tagParam,
    ...
    console.error('[API:public:ads] Error:', error);
