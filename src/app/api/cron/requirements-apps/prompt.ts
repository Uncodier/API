...
7. ONLY if ALL items in the backlog are completely done AND there are no new requests in the instructions, call \`requirement_status\` with \`stage='done'\` and \`message='Project complete'\` to finalize the workflow, and DO NOT create an instance plan.
