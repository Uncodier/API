...
${p.isNewBranch ? '- This is a NEW branch — start from the same base repo (package.json at root); add routes under src/app/.' : '- This branch already has code — review the current state before planning.'}
...
- \`requirement.spec.md\` is IMMUTABLE. It is the contract, not a notebook. Do NOT rewrite it. If contract fields change, append to the \`## Revisions\` section ONLY.
...
2. If the backlog is empty, derive a COMPREHENSIVE list of items (as many as needed to fully cover the scope, typically 5-15) DIRECTLY FROM THE "INSTRUCTIONS (raw from DB)" BLOCK ABOVE and \`action='upsert'\` them. These items form the Backlog. Remember the hierarchy: A Requirement has many Backlog Items, and each Backlog Item will later be broken down into an \`instance_plan\` (a sequence of execution steps). Each item needs \`title\`, \`kind\`, \`phase_id\`, \`acceptance[]\`, and \`tier\` ('core' or 'ornamental'). CRITICAL: You MUST eliminate ambiguity. For UI features, explicitly list the exact routes (e.g., \`/dashboard/spaces\`), the navigation flow, and the required components in the acceptance criteria (e.g. "GET /dashboard renders a grid of Shadcn Cards"). For backend, list the exact API endpoints and data schema. Do NOT \`sandbox_read_file\` \`requirement.spec.md\` / \`feature_list.json\` first — those files may not exist yet on a fresh branch; the INSTRUCTIONS block is the authoritative contract for this cycle. If \`requirement.spec.md\` happens to exist, you MAY read it to enrich the items, but treat it as optional.
3. Pick the single next item (WIP=1). Call \`action='start'\` to mark it in_progress.
4. Create the plan: \`instance_plan\` with \`action='create'\`. BREAK DOWN the backlog item into specific, actionable execution steps (e.g., 1. investigate/setup, 2. backend API, 3. frontend UI, 4. integration/tests). Do NOT just copy the item title into a single step. Every step MUST set \`skill\` and \`metadata.backlog_item_id=<id>\`. CRITICAL: Maximize the use of the plan schema. For the overall plan and EVERY step, you MUST provide \`expected_output\`, \`success_criteria\` (array), and \`validation_rules\` (array) to enforce strict quality control. For frontend steps, you MUST explicitly describe the UI layout, components to use (e.g., Shadcn UI Cards, Dialogs, Tables), and responsive behavior in the step instructions. Do not leave UI execution up to interpretation.
