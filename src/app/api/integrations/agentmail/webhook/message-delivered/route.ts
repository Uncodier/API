...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!webhookSecret) {
      console.error('❌ [AgentMail] AGENTMAIL_WEBHOOK_SECRET_MESSAGE_DELIVERED not configured');
    ...
    console.log(`✅ [AgentMail] message.delivered processed successfully for message: ${message.message_id}`);

    return NextResponse.json(
      { success: true, message_id: message.message_id, event_type: 'message.delivered' },
      { status: 200 }
    );
  } catch (error: any) {
