...
export function instancePlanTool(site_id: string, instance_id: string, user_id?: string) {
  return {
    name: 'instance_plan',
    description:
      'Manage instance plans. Plans are strict execution paths composed of steps that the system delegates to specialized sub-agents. Use action="create" to define a new plan — each step MUST include a "role" (frontend, backend, devops, content, investigate, validate) so the system injects the right skill. Use action="list" to get current plans. Use action="update" to add steps or modify an existing plan. The system auto-executes pending steps as sub-agents after you finish planning.',
    parameters: {
      type: 'object',
      properties: {
        action: {
          ...
          steps: { 
            ...
            items: { 
              ...
              properties: {
                ...
                role: { type: 'string', enum: ['frontend', 'backend', 'devops', 'content', 'investigate', 'plan', 'validate', 'report'], description: 'REQUIRED. Role for skill injection. Determines which specialized agent runs this step.' },
                skill: { type: 'string', description: 'Explicit skill name to inject (e.g. makinari-rol-frontend). Takes priority over role.' },
              },
