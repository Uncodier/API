...
function slimStep(step: unknown): unknown {
  ...
  const slim: StringMap = {
    ...
    role: src.role,
    skill: src.skill,
    requires_sandbox: src.requires_sandbox,
    requires_browser: src.requires_browser,
    browser_interaction_required: src.browser_interaction_required,
    browser_allowed_domains: src.browser_allowed_domains,
    browser_secret_names: src.browser_secret_names,
  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        success_criteria: { type: 'array', items: { type: 'string' }, description: 'Criteria for successful completion' },
          ...
          steps: { 
            ...
            items: { 
              ...
              properties: {
                ...
                expected_output: { type: 'string', description: 'Concrete terminal output. For structured data, use an object shape with named fields and letters/numbers/underscores only, for example { opportunities: [{ url: "url" }], total_opportunities: 0 }.' },
                ...
                requires_browser: { type: 'boolean', description: 'Explicitly provision the browser tool for this step. Prefer this over relying on instruction keyword detection.' },
                browser_interaction_required: { type: 'boolean', description: 'Require a real click/fill/type/press after the initial browser snapshot and a fresh snapshot/get_text afterward. When omitted, the workflow gate infers this from the instructions.' },
                browser_allowed_domains: {
                  type: 'array',
                  items: { type: 'string' },
                  description: 'Domains trusted to receive credentials through sandbox_browser value_env, for example ["freelancer.com", "*.freelancer.com"].',
                },
