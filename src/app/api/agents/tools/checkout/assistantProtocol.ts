    ...
    parameters: {
      ...
      properties: {
        ...
        lines: {
          type: 'string',
          description:
            'JSON string of order lines. Each line needs: catalogItemId, quantity, and optionally unitPriceOverride. Optional modifiers: [{ catalogItemId, quantity, unitPriceOverride? }] — billed as child sale_order_items. For reservable items (is_reservation=true), you MUST include reservationStart and reservationEnd as UTC ISO from reservations.get_available_slots (do not stamp Z onto a wall-clock hour).',
        },
        order_id: {
          type: 'string',
