...
2. **notify**: Sends a notification to a specific team member. **Note: Notifications are rate-limited to 1 per instance per team member per hour.** You can specify the channels to use (e.g., `["whatsapp", "email", "in_app"]`) and a custom `phone_number`. If no explicit channels are provided, the tool will automatically use the best channel available based on the member's profile:
