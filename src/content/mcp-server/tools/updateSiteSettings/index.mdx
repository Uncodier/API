---
title: Site Settings
description: Get or update the site business settings, context, and copywriting guidelines.
---


# Site Settings

Get or update the site business settings, context (about, industry, etc.), and copywriting guidelines (branding, voice, tone). Use "get" action to retrieve current settings, and "update" action to modify them. Provide only the fields you want to update when using the "update" action.

## Input Schema

| Parameter | Type | Description |
| :--- | :--- | :--- |
| action | string | **REQUIRED**. The action to perform: "get" to retrieve settings, "update" to modify them. |
...
The `site_id` and `action` fields are required when calling via REST. Provide only the fields you want to update — all other fields are optional.
