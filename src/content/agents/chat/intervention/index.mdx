...
```json
{
  ...
  "message_id": "optional-existing-message-uuid",
...
| `conversationId` | `string` | No | The UUID of the conversation. Alias of `conversation_id`. |
| `conversation_id` | `string` | No | Same UUID as `conversationId`. Forwarded to `sendWhatsappFromAgent`. |
| `message_id` | `string` | No | Existing message UUID for retry. Skips insert and reuses that row. |
