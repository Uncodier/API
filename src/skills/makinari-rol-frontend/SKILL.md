...
- If dynamic media or thematic visuals are needed, prefer the absolute Makinari media endpoints for images, videos, and icons (not a relative path on this app). For images, obtain a signed URL with an authenticated `POST https://backend.makinari.com/api/public/image/sign` request containing `{ site_id, prompt, width, height }`, then use the returned `url` as `<img src>`. A direct prompt URL is valid only when the asset was pre-generated and must include `site_id`. Never rely on `Origin` or `Referer` to authorize billable generation.
